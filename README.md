...
Everything under `public/media` is original artwork generated by
`scripts/generate-media.mjs` — layered gradient meshes, film grain and
hand-built vector scenes, rasterised with sharp. Nothing is third-party stock,
so there is no licence to track and no attribution required.

Re-run it with `node scripts/generate-media.mjs`; output is deterministic
(seeded PRNG), so the same commit always produces the same bytes.

The hero loop is a 6-second VP8 WebM (~130 KB) built from generated frames. It
is muted, `playsInline`, and backed by a poster still, so a browser that blocks
autoplay loses nothing.

**Swapping in real photography:** `src/lib/media.ts` carries a `swapHint` on
every tile describing the photo that belongs there and at which aspect ratio.
Drop a file into `public/media`, point `src` at it, update `width`/`height`,
and the layout takes it without any other change.
...
- Landing imagery is original illustration, not photography. Real creator
  photos would make it stronger — see `swapHint` in `src/lib/media.ts`.
